...
namespace net
{
    ...
    public:
        ...
        void operator()(int ec, evhttp_connection *evhttp_con)
        {
            on_accept(ec, evhttp_con);
        }

        struct evhttp *get_evhttp() const
        {
            return p_evhttp;
        }
    ...
    void stream_accept_async(struct evhttp *p_evhttp, std::string addr, int port, std::function<void(int ec, evhttp_connection *evhttp_con)> &&func)
    {
        evconnlistener_cb accept_cb = [](struct evconnlistener *listener_ev, evutil_socket_t fd, struct sockaddr *sa, int socklen, void *arg)
        {
...
namespace http
{
    ...
    public:
        request_cb_obj(on_read_cb &&on_read_)
        {
            on_read = std::move(on_read_);
        }

        void operator()(int ec, evhttp_request *req)
        {
            on_read(ec, req);
        }
    ...
    public:
        bufev_cb_obj(on_write_cb &&on_write_)
        {
            on_write = std::move(on_write_);
        }
    ...
    public:
        ...
        void set_res_body(std::string data)
        {
            response_body = data;
        }
        ...
        evhttp_request *get_request() const
        {
            return req;
        }
    ...
    {
        // std::cout << "[DEBUG][write_async] data: " << data << "req: " << req->get_request() << std::endl;
        ...
        bufferevent_setcb(evcon->bufev,
                          ...
                          write_cb,
                          eventcb_ptr,
                          new bufev_cb_obj(std::move(complete_func)));
...
void event_once_call_write_async(write_asyn_func func, std::shared_ptr<http::request> req, std::string data, std::function<void()> cb)
{
    struct wrapper
    {
...
class http_connection : public std::enable_shared_from_this<http_connection>
{
    ...
    typedef struct wrapper_s
    {
        ...
        wrapper_s(std::shared_ptr<http_connection> p)
        {
            p_http = p;
        }
...
public:
    ...
    do_read()
    {
        http::read_async(this->evhttp_con, std::bind(&http_connection::on_read, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
    }
...
class http_app : public std::enable_shared_from_this<http_app>
{
    ...
    typedef struct wrapper_s
    {
        ...
        wrapper_s(std::shared_ptr<http_app> p)
        {
            p_http = p;
        }
        ~wrapper_s()
...
public:
    ...
    void start()
    {
        net::stream_accept_async(p_evhttp, addr, port, std::bind(&http_app::on_accept, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
    }

    void add_handler(const std::string &route, route_hanhdl_func func_handler_)
    {
        routes[route] = func_handler_;
    }

    evhttp *get_evhttp() const
    {
        return this->p_evhttp;
    }

private:
    void on_accept(int ec, evhttp_connection *evhttp_con)
    {
        auto p = std::shared_ptr<http_connection>(new http_connection(evhttp_con, std::bind(&http_app::route_handler, http_app::shared_from_this(), std::placeholders::_1)), [](const auto p) { /*std::cout << "[DEBUG] http_connection is deleted. " << std::endl; */ delete p; });
        p->start();
    }
...
// helper function
std::shared_ptr<http_app>
make_http(event_base *base, const std::string &addr, uint16_t port)
{

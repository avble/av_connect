...
public:
  ...
  template <class T>
  message(std::weak_ptr<T> connect_, boost::asio::const_buffer _buffer,
  ...
  std::string_view data() const {
    return std::string_view{(char *)buffer_read.data(), buffer_read.size()};
  }
...
public:
  ...
  explicit ws_session(tcp::socket &&socket,
                      std::function<void(message)> _message_handler)
  ...
  void run() {
    net::dispatch(
        ws_.get_executor(),
        beast::bind_front_handler(&ws_session::on_run, shared_from_this()));
  }
  ...
  void do_read() {
    ...
    input_buffer.clear();
    ws_.async_read(input_buffer, beast::bind_front_handler(&ws_session::on_read,
                                                           shared_from_this()));
  }
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    message_handler(message{std::weak_ptr<ws_session>{shared_from_this()},
                            input_buffer.data(), output_buffer});
  }
...
public:
  server(net::io_context &ioc, tcp::endpoint endpoint,
         std::function<void(ws::message)> _message_handler)
...
auto make_server(unsigned short port,
                 std::function<void(ws::message)> message_handler,
                 std::function<void(beast::error_code)> on_event_cb) {
  const boost::asio::ip::address address =
      boost::asio::ip::make_address("0.0.0.0");
  auto server_ =
      server{io_context::ioc, tcp::endpoint{address, port}, message_handler};
  server_.run();
  return server_;
}
...
void start_server(unsigned short port,
                  std::function<void(ws::message)> message_handler) {
  auto server_ =
      make_server(port, message_handler, [](beast::error_code ec) {});
  server_.run();
  io_context::ioc.run();
}
...
public:
  ...
  explicit ws_client(net::io_context &ioc,
                     std::function<void(std::string message)> _on_message)
  ...
  void run(char const *host, char const *port, char const *text) {
    ...
    resolver_.async_resolve(
        host, port,
        beast::bind_front_handler(&ws_client::on_resolve, shared_from_this()));
  }
  ...
  void on_resolve(beast::error_code ec, tcp::resolver::results_type results) {
    ...
    beast::get_lowest_layer(ws_).async_connect(
        results,
        beast::bind_front_handler(&ws_client::on_connect, shared_from_this()));
  }
  ...
  void on_connect(beast::error_code ec,
    ...
    ws_.async_handshake(host_, "/",
                        beast::bind_front_handler(&ws_client::on_handshake,
                                                  shared_from_this()));
  }
  ...
  void do_write(std::string message) {
    ws_.async_write(
        net::buffer(message),
        beast::bind_front_handler(&ws_client::on_write, shared_from_this()));
  }
  ...
  void do_read() {
    ws_.async_read(buffer_, beast::bind_front_handler(&ws_client::on_read,
                                                      shared_from_this()));
  }
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    // beast::bind_front_handler(&ws_client::on_close, shared_from_this()));
  }
...
//     std::make_shared<ws_client>(ioc)->run(_host.c_str(), port.c_str(),
//     "text");
...
std::shared_ptr<ws_client>
make_client(std::string host, std::string port,
            std::function<void(std::string message)> on_read_) {
